...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { padding-bottom: 12px; margin-bottom: 0; border-bottom: 3px solid {{ $cp }}; }
...
<body>
        ...
        <table>
            <tr>
                ...
                <td style="vertical-align:middle;">
                    ...
                    <div class="subtitulo">Reporte de Médicos</div>
                    @if($empresa->nit)
    ...
    {{-- ── META-INFO ── --}}
    <div class="meta-bar">
        <strong>Total registrados:</strong> {{ $medicos->count() }} médicos
    </div>
    ...
    <table class="datos">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $m->usuario?->identificacion ?? '—' }}</td>
                <td style="color:#6b7280;">{{ $m->usuario?->email ?? '—' }}</td>
                <td>{{ $m->especialidad ?? '—' }}</td>
                <td>{{ $m->registro_medico ?? '—' }}</td>
                <td>
                    <span class="badge {{ $m->usuario?->activo ? 'badge-si' : 'badge-no' }}">
                        {{ $m->usuario?->activo ? 'Sí' : 'No' }}
                    </span>
                </td>
            ...
            @empty
            <tr>
                <td colspan="8" style="text-align:center; color:#9ca3af; padding:16px;">Sin médicos registrados.</td>
            </tr>
            @endforelse
        </tbody>
        @if($medicos->count() > 0)
        <tfoot>
            <tr>
                <td colspan="8">Total: {{ $medicos->count() }} registros</td>
            </tr>
        </tfoot>
        @endif
    </table>
