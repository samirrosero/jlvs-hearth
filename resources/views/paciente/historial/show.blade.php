    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-6 bg-gray-50 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-bold text-gray-800">Historia Clínica #{{ str_pad($historia->id, 8, '0', STR_PAD_LEFT) }}</h2>
    ...
    @if ($sv)
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
                @foreach ([
                    ['Peso', $sv->peso_kg, 'kg'],
                    ['Talla', $sv->talla_cm, 'cm'],
                    ['P. Sistólica', $sv->presion_sistolica, 'mmHg'],
                    ['P. Diastólica', $sv->presion_diastolica, 'mmHg'],
                    ['Temperatura', $sv->temperatura_c, '°C'],
                    ['Frec. Cardiaca', $sv->frecuencia_cardiaca, 'lpm'],
                    ['SpO₂', $sv->saturacion_oxigeno, '%'],
                    ['Frec. Respiratoria', $sv->frecuencia_respiratoria, 'rpm'],
                ] as [$label, $valor, $unidad])
                    @if ($valor !== null)
                        <div class="text-center p-3 bg-blue-50 rounded-xl border border-blue-100">
                            <p class="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{{ $label }}</p>
