            ...
            <form method="POST" action="{{ route('registro.empleador', request()->only(['empresa', 'nit'])) }}" enctype="multipart/form-data">
                ...
                <div class="grid-2" x-data="{ rolSeleccionado: '' }">
                    ...
                    <div class="field span-2">
                        ...
                        <div x-show="rolSeleccionado === 'medico'" x-cloak class="mt-4 space-y-4">
                            <div class="field">
                                <label>Especialidad <span class="text-red-500">*</span></label>
                                <input type="text" name="especialidad" placeholder="Ej: Cardiología, Pediatría..."
                                       :required="rolSeleccionado === 'medico'">
                            </div>
                            <div class="field">
                                <label>Número de tarjeta profesional <span class="text-red-500">*</span></label>
                                <input type="text" name="numero_tarjeta_profesional" placeholder="Ej: TP-123456"
                                       :required="rolSeleccionado === 'medico'">
                                <p class="text-xs text-gray-400 mt-1">Será verificado en RETHUS</p>
                            </div>
                        </div>

                        {{-- Campo de documentación para Gestores y Admins --}}
                        <div x-show="rolSeleccionado === 'gestor_citas' || rolSeleccionado === 'administrador'" x-cloak class="mt-4">
                            <div class="field">
                                <label>Cargo o experiencia previa</label>
                                <input type="text" name="cargo_experiencia" placeholder="Ej: Enfermera con 5 años de experiencia...">
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Foto del diploma (solo médicos) - OPCIONAL por ahora --}}
                    <div class="field span-2" x-show="rolSeleccionado === 'medico'" x-cloak>
                        <label>Foto del diploma o tarjeta profesional (opcional)</label>
                        <div class="upload-doc" onclick="document.getElementById('foto_diploma').click()">
                            <img id="diploma-preview" src="" alt="Vista previa" style="display:none;">
                            <svg class="w-8 h-8 mx-auto text-slate-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" id="diploma-icon">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                      d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/>
                            </svg>
                            <p class="text-xs text-slate-500 font-medium" id="diploma-text">Haz clic para subir diploma</p>
                            <p class="text-xs text-slate-400 mt-0.5">JPG, PNG, WEBP · máx. 3 MB</p>
                            <input type="file" id="foto_diploma" name="foto_diploma"
                                   accept="image/jpg,image/jpeg,image/png,image/webp"
                                   onchange="previewDiploma(this)">
                        </div>
                    </div>

                    {{-- Documento de acreditación (opcional para todos) --}}
                    <div class="field span-2">
                        <label>Documento de acreditación o certificación (opcional)</label>
                        <div class="upload-doc" onclick="document.getElementById('documento_acreditacion').click()">
                            <img id="acreditacion-preview" src="" alt="Vista previa" style="display:none;">
                            <svg class="w-8 h-8 mx-auto text-slate-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" id="acreditacion-icon">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                      d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                            </svg>
                            <p class="text-xs text-slate-500 font-medium" id="acreditacion-text">Haz clic para subir certificación adicional</p>
                            <p class="text-xs text-slate-400 mt-0.5">JPG, PNG, WEBP · máx. 3 MB</p>
                            <input type="file" id="documento_acreditacion" name="documento_acreditacion"
                                   accept="image/jpg,image/jpeg,image/png,image/webp"
                                   onchange="previewAcreditacion(this)">
                        </div>
                    </div>
...
function previewDiploma(input) {
    ...
    const reader = new FileReader();
    reader.onload = e => {
        const img = document.getElementById('diploma-preview');
        const icon = document.getElementById('diploma-icon');
        const txt  = document.getElementById('diploma-text');
        img.src = e.target.result;
        img.style.display = 'block';
        icon.style.display = 'none';
        txt.textContent = input.files[0].name;
...
function previewAcreditacion(input) {
    if (!input.files?.[0]) return;
    const reader = new FileReader();
    reader.onload = e => {
        const img = document.getElementById('acreditacion-preview');
        const icon = document.getElementById('acreditacion-icon');
        const txt  = document.getElementById('acreditacion-text');
        img.src = e.target.result;
        img.style.display = 'block';
        icon.style.display = 'none';
        txt.textContent = input.files[0].name;
